...
class nixlGusliEngine : public nixlBackendEngine {
 ...
 public:
	...
	~nixlGusliEngine();
	bool supportsNotif( void) const override { return false; }
	bool supportsRemote(void) const override { return false; }
	bool supportsLocal( void) const override { return true; }		// Nixel client abscures the server so it acts as local, without remote
	bool supportsProgTh(void) const override{ return false;	}
	nixl_mem_list_t getSupportedMems(void) const override { return __getSupportedGusliMems(); }
	...
	nixl_status_t loadLocalMD(nixlBackendMD* in, nixlBackendMD* &out) override { out = in; return NIXL_SUCCESS; }
	...
	[[nodiscard]] nixl_status_t deregisterMem(nixlBackendMD *out) override;
