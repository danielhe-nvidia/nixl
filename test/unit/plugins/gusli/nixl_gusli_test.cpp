...
#define QUIT_ON_ERR(msg, status)                                                             \
    ...
    run_write_read_verify (void) {
        ...
        print_segment_title (phase_title (
            absl::StrFormat ("1[xfer] Write-Read-Verify %u[KB]", transfer_size >> 10)));
        if (1) {
            ...
            for (int i = 0; i < 2; i++, treq = nullptr) {
                ...
                status = agent.createXferReq (
                    io_phases[i], bdev_io_1src, bdev_io_1dst, agent_name, treq);
            ...
            if (!test_pattern.verify ((char *)bdev_io_1src[0].addr, transfer_size))
                return -__LINE__;
        ...
        if (1) {
            for (int with_sgl = 1; with_sgl >= 0;
                ...
                for (int i = 0; i < 2; i++, treq = nullptr) {
                    const std::string io_t_str = nixlEnumStrings::xferOpStr (io_phases[i]);
                    print_segment_title (
                        phase_title (absl::StrFormat ("%s Test, nIOs=%u, with_sgl=%c",
                                                      io_t_str.c_str(),
                                                      n_ranges,
                                                      (with_sgl ? 'Y' : 'N'))));
